...
class ELBMetrics < Sensu::Plugin::Metric::CLI::Graphite
  ...
  option :engine_name,
    :description => "Name of the RDS Load Balancer",
    :short => "-n RDS_NAME",
    :long => "--name RDS_NAME"
  ...
  def run
    ...
    if config[:scheme] == ""
      graphitepath = "#{config[:engine_name]}"
    else
    ...
    begin
      ...
      metrics.each do |key, value|
        ...
        unless data.nil?
          ...
          output "#{graphitepath}.#{key.downcase}", data[value], data['Timestamp'].to_i
        end
      end
    rescue Exception => e
